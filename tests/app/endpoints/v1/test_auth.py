...
class TestAuthenticationHandler(BaseHandler):
    ...
    async def test_authentication_no_user(self, client, potential_user):
        response = await client.post(
            url=self.get_url("authentication"), data=self.get_data(potential_user.username, potential_user.password)
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, response.json()

    async def test_authentication_wrong_password(self, client, created_user, potential_user):
        response = await client.post(
            url=self.get_url("authentication"), data=self.get_data(potential_user.username, "wrong_password")
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, response.json()

    async def test_authentication_success(self, client, created_user, potential_user):
        response = await client.post(
            url=self.get_url("authentication"),
            data=self.get_data(potential_user.username, potential_user.password),
        )
...
class TestGetMeHandler(BaseHandler):
    ...
    async def test_get_me_bad_token_no_user(self, client, potential_user):
        response = await client.get(
            url=self.get_url("me"),
            headers={"Authorization": f"Bearer {user.create_access_token(data={'sub': potential_user.username})}"},
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED, response.json()
